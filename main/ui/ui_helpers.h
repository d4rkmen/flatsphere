...
#define UI_MOVE_CURSOR_LEFT 3
    ...
    void ui_save_time_to_rtc(int hour, int minute);
    void ui_load_datetime_from_rtc(struct tm* timeinfo);
    void ui_save_date_to_rtc(int day, int month, int year);

    // Timezone management functions
    uint8_t ui_get_current_timezone_index(void);
    uint8_t ui_get_timezone_count(void);
    const char* ui_get_timezone_name(uint8_t index);
    const char* ui_get_timezone_city(uint8_t index);
